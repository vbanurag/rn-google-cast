...
public class RNRemoteMediaClientListener  implements RemoteMediaClient.Listener, RemoteMediaClient.ProgressListener {
    private RnGoogleCastModule module;
    private boolean playbackStarted;
    private boolean playbackEnded;
    private int currentItemId;

    public RNRemoteMediaClientListener(RnGoogleCastModule module) {
        this.module = module;
    }
    ...
    public void onStatusUpdated() {
        module.runOnUiQueueThread(new Runnable() {
            ...
            public void run() {
                ...
                module.emitMessageToRN(RnGoogleCastModule.MEDIA_STATUS_UPDATED,
                        prepareMessage(mediaStatus));

                if (!playbackStarted &&
                        mediaStatus.getPlayerState() == MediaStatus.PLAYER_STATE_PLAYING) {
                    module.emitMessageToRN(RnGoogleCastModule.MEDIA_PLAYBACK_STARTED,
                            prepareMessage(mediaStatus));
                ...
                if (!playbackEnded &&
                    ...
                    module.emitMessageToRN(RnGoogleCastModule.MEDIA_PLAYBACK_ENDED,
                            prepareMessage(mediaStatus));
                    playbackEnded = true;
                }
            }
            ...
            public void run() {
                ...
                if (mediaStatus.getPlayerState() == MediaStatus.PLAYER_STATE_PLAYING) {
                    module.emitMessageToRN(
                            RnGoogleCastModule.MEDIA_PROGRESS_UPDATED,
